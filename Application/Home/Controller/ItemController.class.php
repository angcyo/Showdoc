...
class ItemController extends BaseController
{
    ...
    private function _show_regular_item($item)
    {
        ...
        if ($keyword) {
            ...
            foreach ($pages as $key => $value) {
                //修改一级页面标题
                $pages[$key] = $this->changePageTitle($pages[$key]);
            }
            //获取所有二级目录
            ...
            if ($catalogs) {
                foreach ($catalogs as $key => &$catalog) {
                    ...
                    foreach ($catalog['pages'] as $key => $value) {
                        //修改二级页面标题
                        $catalog['pages'][$key] = $this->changePageTitle($catalog['pages'][$key]);
                    }
                    ...
                    if ($catalog['catalogs']) {
                        ...
                        foreach ($catalog['catalogs'] as $key3 => &$catalog3) {
                            ...
                            foreach ($catalog['pages'] as $key4 => $value4) {
                                //修改三级页面标题
                                $catalog3['pages'][$key4] = $this->changePageTitle($catalog3['pages'][$key4]);
                            }
    ...
    private function changePageTitle($page)
    {
        $addTime = $page['addtime'];
        $nowTime = time();
        $title = $page['page_title'];

        if ($nowTime - $addTime < 86400 * 1) {
            $page['page_title'] = $title . ' 今天';
        } else if ($nowTime - $addTime < 86400 * 2) {
            $page['page_title'] = $title . "  昨天";
        } else if ($nowTime - $addTime < 86400 * 3) {
            $page['page_title'] = $title . "  " . date("m-d", $addTime);
        } else {
            $page['page_title'] = $title . " " . date("m-d", $addTime);
        }
        return $page;
